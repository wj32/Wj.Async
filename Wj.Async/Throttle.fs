...
module Throttle =
  ...
  // IThrottle functions
  let enqueue (t : IThrottle) f = t.Enqueue(f)

  module OnException =
    type T = Stop | Continue

  [<AbstractClass>]
  type QueueThrottle private(uponException : exn -> unit) =
    let childSupervisor = ChildSupervisor.create "QueueThrottle"
  ...
  type ParallelThrottle =
    | Unique

    interface IThrottle with
      member this.Enqueue(f) = f ()

  type ParallelAtMostThrottle(maxTasks, onException) =
    inherit QueueThrottle(onException)
  ...
  type ParallelAtMostStartedPerThrottle(maxTasks, span : TimeSpan, onException) =
    inherit QueueThrottle(onException)
  ...
  type SequentialAtMostOneStartedPerThrottle(span : TimeSpan, onException) =
    inherit QueueThrottle(onException)
  ...
  let sequentialThrottle () = ParallelAtMostThrottle(1, OnException.Stop) :> IThrottle

  let sequential () = Parallelism.Throttle (sequentialThrottle ())

  let parallelThrottle () = ParallelThrottle.Unique :> IThrottle

  let ``parallel`` () = Parallelism.Throttle (parallelThrottle ())

  let parallelAtMostThrottle maxTasks onException =
    ParallelAtMostThrottle(maxTasks, onException) :> IThrottle

  let parallelAtMost maxTasks =
    Parallelism.Throttle (parallelAtMostThrottle maxTasks OnException.Stop)

  let parallelAtMostStartedPerThrottle maxTasks span onException =
    ParallelAtMostStartedPerThrottle(maxTasks, span, onException) :> IThrottle

  let parallelAtMostStartedPer maxTasks span =
    Parallelism.Throttle (parallelAtMostStartedPerThrottle maxTasks span OnException.Stop)

  let sequentialAtMostOneStartedPerThrottle span onException =
    SequentialAtMostOneStartedPerThrottle(span, onException) :> IThrottle

  let sequentialAtMostOneStartedPer span =
    Parallelism.Throttle (sequentialAtMostOneStartedPerThrottle span OnException.Stop)
